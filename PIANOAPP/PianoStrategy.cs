...
namespace PIANOAPP
{
    public class PianoStrategy:ISoundStrategy
    {
        ...
        public void PlayTone(MusicElement element, MidiOut midiOut)
        {
            ...
            {
                // Czekaj przez czas trwania pauzy
                Thread.Sleep(pause.Duration);
            }

            Console.WriteLine($"Element {element.GetType().Name} został odtworzony.");
        }
        public void StartTone(Note note, MidiOut midiOut)
        {
            try
            {
                midiOut.Send(MidiMessage.ChangePatch(1, 1).RawData);
                midiOut.Send(MidiMessage.StartNote(note.MidiNum, 127, 1).RawData);
                Console.WriteLine("Dźwięk fortepianu został odtworzony.");
            }
            catch (Exception ex)
            {
